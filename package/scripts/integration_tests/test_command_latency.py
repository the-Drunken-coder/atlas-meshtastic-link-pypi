...
def main() -> int:
    ...
    try:
        ...
        print(f"[latency] Created task {task_id}; waiting for world_state update at {world_state_path}")
        ...
        print(f"[latency] Task {task_id} visible in world_state. latency_ms={latency_ms:.2f}")
        return 0
    except Exception as exc:
        print(f"[latency] ERROR: {exc}")
        return 1
    finally:
