...
def test_gateway_asset_roundtrip(two_radio_ports: tuple[str, str]):
    """End-to-end smoke: asset and gateway radios exchange direct messages."""
    ...
    async def _run() -> None:
        ...
        try:
            ...
            payload_to_gateway = f"ATLAS_HW_SMOKE_A2G::{uuid.uuid4().hex}".encode("utf-8")
            await asset_radio.send(payload_to_gateway, destination=gateway_id)
            received_payload, sender = await _await_payload(gateway_radio, payload_to_gateway)
            assert received_payload == payload_to_gateway
            assert sender == asset_id

            payload_to_asset = f"ATLAS_HW_SMOKE_G2A::{uuid.uuid4().hex}".encode("utf-8")
            await gateway_radio.send(payload_to_asset, destination=asset_id)
            received_payload, sender = await _await_payload(asset_radio, payload_to_asset)
            assert received_payload == payload_to_asset
